...
pub async fn analyze_comment(comment: &str) -> Result<AnalyzeResponse, CustomError> {
    ...
    let normalized_comment = remove_all_whitespace(&normalize_fancy_text(comment));
    {
        ...
        for (keyword, confidence) in cache.iter() {
            ...
            if normalized_comment.contains(&keyword.to_lowercase()) {
    ...
    println!("DeepSeek API response: {}", body);
    ...
    let parsed_response: serde_json::Value = serde_json::from_str(&body)?;
    ...
    let spam: i32 = parts[0].parse().map_err(|_| "Invalid spam value")?;
    let keyword = parts[1].to_string();
    let confidence: f64 = parts[2].parse().map_err(|_| "Invalid confidence value")?;
